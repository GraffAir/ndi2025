...
class LogicielCategorieModel {
  ...
  findById(id) {
    return new Promise((resolve, reject) => {
      this.db.get(`SELECT * FROM ${this.tableName} WHERE software_id = ?`, [id], (err, row) => {
        if (err) reject(err);
        else resolve(row);
      });
    });
...
module.exports = LogicielCategorieModel;
