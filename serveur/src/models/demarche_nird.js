...
class Demarche_nirdModel {
  ...
  findById(id) {
    return new Promise((resolve, reject) => {
      this.db.get(`SELECT * FROM ${this.tableName} WHERE rowid = ?`, [id], (err, row) => {
        if (err) reject(err);
        else resolve(row);
      });
    });
...
module.exports = Demarche_nirdModel;
